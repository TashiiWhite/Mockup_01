...
function Item({ item, isOpen, onToggle, index }) {
  ...
  return (
    <div className="reveal border-b border-ink/10">
      <h3>
        <button
          id={btnId}
      ...
      >
        <div className="min-h-0">
          <p className="max-w-2xl pb-6 leading-relaxed text-stone">{item.a}</p>
        </div>
      </div>
    </div>
  )
    ...
    <section id="faq" className="section-pad">
      <div className="mx-auto max-w-3xl px-6 md:px-10">
        <div className="reveal text-center">
          ...
          <h2 className="mt-4 font-serif text-4xl text-ink md:text-5xl">{faq.title}</h2>
        </div>
